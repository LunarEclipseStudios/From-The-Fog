...
#tooFarOrCloseKill
execute as @e[type=armor_stand,tag=spread] at @s unless entity @a[distance=..40] run kill @s
execute as @e[type=armor_stand,tag=spread] at @s if entity @a[distance=..15] run kill @s

#createModel
schedule function watching:events/sightings/setup/models 5t

#debugMode
tellraw @a[tag=debugMode] ["",{"text":"Attempted to spawn a stalking Herobrine on ","color": "green"},{"selector":"@s","color": "green"},{"text":" at ","color": "green"},{"nbt":"Pos","entity":"@e[type=armor_stand,tag=spread]","color": "dark_green"}]
