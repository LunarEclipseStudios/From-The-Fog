...
$data modify storage lunareclipse.watching:config_options config.settings.option.$(option).$(page) set value {id: "$(option)", type: "$(type)", options: $(options), page: $(page), offset: "$(option_offset)"}