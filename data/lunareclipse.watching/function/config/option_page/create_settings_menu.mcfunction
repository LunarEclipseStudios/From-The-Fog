...
$execute if data storage lunareclipse.watching:config_options config.settings.list.$(id).page_list[1] at @e[type=item_display,tag=watching.menu,tag=watching.settings,limit=1] positioned ^3.15 ^ ^ positioned ~ ~1.6 ~ run summon item_display ~ ~ ~ {Tags:["watching.config","watching.settings","watching.button"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[1f,1f,1f]},brightness:{sky:15,block:15},item:{id:"minecraft:white_dye",count:1,components:{"minecraft:custom_data":{type:"settings",current_page:$(page),id:"$(id)",function:"lunareclipse.watching:config/next_page/switch_page"},"minecraft:item_model":"lunareclipse.watching:ui/config/next_page"}}}
