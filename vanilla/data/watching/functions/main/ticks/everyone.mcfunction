#spottedCreeping
execute if entity @e[type=armor_stand,tag=creeping,distance=..60] unless entity @s[gamemode=spectator] unless score @s crash matches 1.. anchored eyes facing entity @e[type=armor_stand,sort=nearest,limit=1,tag=spread] eyes anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.85] anchored eyes facing entity @e[type=armor_stand,sort=nearest,limit=1,tag=spread] eyes positioned ^ ^ ^ run function watching:events/sightings/spotted
#spottedStalking
...
execute as @s[gamemode=!spectator] at @s unless score @s crash matches 1.. if entity @e[type=armor_stand,tag=spread,tag=creeping,distance=..1.5] if entity @e[type=armor_stand,tag=herobrineModel,distance=..5] run scoreboard players add @s crash 1
